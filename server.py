...
DRAFTING_URL = "http://127.0.0.1:8772"
...
SERVICES = [
    ...
    },
    {
        ...
        "launch_hint": r"X:\Github\drafting-room\run_hidden.vbs",
        "order": 7,
    },
]
...
async def tick_draftingroom(client: httpx.AsyncClient) -> None:
    """The drafting room is a workbench, not a dispatch source: a health lamp
    and the number of drafts on the board. No Ledger lines -- it never notifies.
    ...
    src = SOURCES["draftingroom"]
    try:
        resp, ms = await _timed_get(
            client, f"{DRAFTING_URL}/api/v1/app-conversations/count")
        src.state, src.latency_ms, src.note = "open", ms, None
        n = int(resp.json() or 0)
        src.stat = {"drafts": n} if n else {}
    except Exception as exc:
        src.state = "dark"
        src.latency_ms = None
        src.stat = {}
        log.debug("drafting room dark: %s", exc)
