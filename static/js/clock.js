...
/* Two lines engraved on the dial's inner flange, just inside the gilt rule:
   set as one run centred under six, the second after a space of two
   characters, tops toward the arbor, reading left to right along the bottom
   of the dial, 11 units high, so from any distance they are a fine engraved
   band and close up they read. The glyphs are outlines baked from Yu Mincho
   Demibold (a 1000-unit box, baseline at 1000), so they do not depend on
   any face the page loads. */
var DED_R = 397.5, DED_SIZE = 11, DED_TRACK = 1.2, DED_GAP = 2;
function dedication() {
  ...
  var n0 = DEDICATION[0].length, n1 = DEDICATION[1].length;
  var start = 180 + (n0 + DED_GAP + n1) * step / 2;       // the run's left end
  lay(DEDICATION[0], start - 0.5 * step);
  lay(DEDICATION[1], start - (n0 + DED_GAP + 0.5) * step);
